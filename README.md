...
- It's never guaranteed that the metadata of the audio file is truthful, as some people like to upsample and re-encode lower "quality" and MP3s into FLACs with a much higher bit depth and sample rate. For this, one simple tool for manually checking is: [Spek](https://www.spek.cc/). It's super simple, free, and open source. Drag an audio file into the window, and it'll show you the spectrogram. This displays sample rate mainly. If the sample rate says it's 44.1kHz, ensure the "energy" shows up to half of the sample rate (~22kHz). Even if the energy doesn't reach the value you're looking for, so long as the "cut off" seems fuzzy, it may still be a valid file. Hard, flat, wall cutoffs may indicate upsampling.
